...
export interface InvoicesFiltersUI {
    status: string[]
...
interface InvoicesFilterBarProps {
    filters: InvoicesFiltersUI
    onFiltersChange: (changes: Partial<InvoicesFiltersUI>) => void
}

const statusOptions: { value: string; label: string; color: string }[] = [
    { value: 'draft', label: 'مسودة', color: 'gray' },
    { value: 'pending', label: 'معلّقة', color: 'amber' },
    { value: 'paid', label: 'مدفوعة', color: 'green' },
    { value: 'partially_paid', label: 'مدفوعة جزئياً', color: 'blue' },
...
const InvoicesFilterBar: React.FC<InvoicesFilterBarProps> = ({ filters, onFiltersChange }) => {
    const defaultFilters: InvoicesFiltersUI = { status: [], paymentMethod: [], dateFrom: '', dateTo: '', teamId: '', search: '', mismatchOnly: false }
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 px-4 py-3 border-b border-gray-100">
                <div className="flex items-center justify-between">
                    ...
                    >
                        ...
                        <h3 className="text-base font-semibold">فلاتر الفواتير</h3>
                        <svg
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center space-x-1.5 space-x-reverse">
                                ...
                                <label className="text-xs font-semibold text-gray-700 uppercase tracking-wide">حالة الفاتورة</label>
                            </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    <select
                                        className={`w-full px-3 py-2 text-xs bg-white border border-gray-200 rounded-md focus:border-blue-500 focus:ring-1 focus:ring-blue-200 transition-all duration-150 appearance-none ${loadingTeams ? 'opacity-50 cursor-not-allowed' : 'hover:border-gray-300'
                                            }`}
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full pr-9 pl-3 py-2 text-xs bg-white border border-gray-200 rounded-md focus:border-blue-500 focus:ring-1 focus:ring-blue-200 transition-all duration-150 hover:border-gray-300"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center space-x-1.5 space-x-reverse">
                                    <div className="w-1.5 h-1.5 bg-amber-500 rounded-full"></div>
                                    <label className="text-xs font-semibold text-gray-700 uppercase tracking-wide">فلتر التحذيرات</label>
                                </div>
                                <button
                                    type="button"
                                    onClick={() => onFiltersChange({ mismatchOnly: !filters.mismatchOnly })}
                                    className={`w-full flex items-center gap-3 p-2.5 rounded-md border cursor-pointer transition-all duration-150 hover:shadow-sm ${filters.mismatchOnly
                                            ? 'border-amber-400 bg-amber-50'
                                            : 'border-gray-200 bg-white hover:border-gray-300'
                                        }`}
                                >
                                    <div className={`flex-shrink-0 w-4 h-4 rounded border flex items-center justify-center transition-all duration-150 ${filters.mismatchOnly
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        <svg className={`w-3.5 h-3.5 ${filters.mismatchOnly ? 'text-amber-600' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4.5c-.77-.833-2.694-.833-3.464 0L3.34 16.5c-.77.833.192 2.5 1.732 2.5z" />
                                        </svg>
                                        <span className={`text-xs font-medium ${filters.mismatchOnly ? 'text-amber-700' : 'text-gray-700'
                                            }`}>
                                            فواتير غير متطابقة مع الطلب
                                        </span>
                                    </div>
                                </button>
                            </div>
